...
Dir.mkdir("projects/#{project_name}/shared_reports") unless File.directory?("projects/#{project_name}/shared_reports")
shared_report_file_names = Dir.entries("projects/#{project_name}/shared_reports").select {|file| /csv/.match(file) }
shared_report_file_paths = shared_report_file_names.map {|name| "projects/#{project_name}/shared_reports/#{name}"}
...
pulled_data_file_names = Dir.entries("projects/#{project_name}/pulled_data").select {|file| /yml/.match(file) }
pulled_data_file_paths = pulled_data_file_names.map {|name| "projects/#{project_name}/pulled_data/#{name}"}

File.open("projects/#{project_name}/blacklist.yml", 'w') {|f| f.puts [/mortician/].to_yaml} unless File.exist?("projects/#{project_name}/blacklist.yml")
blacklisted_regex = YAML.safe_load_file("projects/#{project_name}/blacklist.yml", permitted_classes: [Regexp])
...
output_filepath = "projects/#{project_name}/report_in_progress.csv"
